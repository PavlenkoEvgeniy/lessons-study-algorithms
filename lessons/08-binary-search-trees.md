...
```
✅ ХОРОШО:
   - Нужен быстрый поиск, вставка И удаление одновременно
   - Данные должны быть отсортированы при обходе (in-order)
   - Нужно быстро находить минимум/максимум

❌ ПЛОХО:
   - Данные приходят уже отсортированными (дерево вырождается в список)
   - Нужен гарантированный O(log n) всегда (тогда нужно сбалансированное дерево)
   - Данных мало — массив или хеш-таблица проще
```
